...
async def new_pair(event):
    ...
    if api.get_verified(token_address, "bsc"):
    ...
    if pool == 0 or pool == "" or not pool:
        ...
        pool_dollar = float(pool) * float(api.get_native_price("bnb"))
        pool_text = (
    ...
    for chat_id in channel_chat_ids:
        ...
        try:
            if event["args"]["token0"] == ca.WBNB or event["args"]["token1"] == ca.WBNB:
                image_url = api.get_token_image(token_address, "bsc")
                if image_url is None:
                    image_url = "N/A"

                db.token_add(token_name[1], event["args"]["pair"], token_address, "bsc", image_url)
